...
where  device_type in ('GPS','OBD') and product_type in ('硬件','服务','安装') and business_type ='SAAS系统订阅服务—旧版本' and finance_time >= '2024-01-01' and total > 0
...
sum(cost) AS costx 
 from dj_2024_settle_details where settle_time >= '2024-01-01' and device_type = 8
...
SELECT TRIM(cus_name) as cus_name, substr(finance_time,1,7) as monthx, sale_type,
SUM(case when product_type='硬件' then total else 0 end) as total,SUM(income) as income,SUM(cost) AS costx FROM dj_2024_settlment_order_finance
where  device_type in ('记录仪','车充') and product_type in ('硬件','服务','安装') and finance_time >= '2024-01-01' and total > 0 and business_type ='SAAS系统订阅服务—旧版本'
...
SUM(case when install_price is NULL then 0 else install_price end) ) as income,
sum(cost) AS costx 
 from dj_2024_settle_details_record where settle_time >= '2024-01-01' and  device_type in (6, 12501)
GROUP BY settle_merchant,DATE_FORMAT(settle_time,'%Y-%m')
)
as B ON (B.cus_name = A.cus_name AND a.monthx = B.monthx) HAVING abs(gapincome) >1.0;
gap <> 0
...
SUM(case when install_price is NULL then 0 else install_price end) ) as income,
sum(cost) AS costx 
 from dj_2024_settle_details_return where settle_time >= '2024-01-01' and device_type in (6, 12501)
GROUP BY settle_merchant,DATE_FORMAT(settle_time,'%Y-%m')
)
as B ON (B.cus_name = A.cus_name AND a.monthx = B.monthx) HAVING abs(gapincome) >1.0;
gap > 0
...
SUM(case when install_price is NULL then 0 else install_price end) ) as income,
sum(cost) AS costx 
 from dj_2024_settle_details_return where settle_time >= '2024-01-01' and device_type in (8)
GROUP BY settle_merchant,DATE_FORMAT(settle_time,'%Y-%m')
)
as B ON (B.cus_name = A.cus_name AND a.monthx = B.monthx) HAVING  abs(gapincome) >1.0;
gap <> 0
...
SUM(case when install_price is NULL then 0 else install_price end) ) as income,
sum(cost) AS costx 
 from dj_2024_settle_details where settle_time >= '2024-01-01' and device_type in (8) and settle_merchant = '广汇汽车'
GROUP BY settle_merchant,DATE_FORMAT(settle_time,'%Y-%m')
)
as B ON (B.cus_name = A.cus_name AND a.monthx = B.monthx) HAVING  abs(gapcost) >1.0;
gap <> 0
abs(gapincome) >1.0
abs(gapcost) >1.0
...
SELECT '广汇汽车' as cus_name, substr(finance_time,1,7) as monthx, sale_type,
SUM(case when product_type='硬件' then total else 0 end) as total,SUM(income) as income,SUM(cost) AS costx  FROM dj_2024_settlment_order_finance
where  device_type in ('记录仪','车充') and product_type in ('硬件','服务') and finance_time >= '2024-01-01' and total > 0 and business_type ='SAAS系统订阅服务—旧版本'
and cus_name in ('东台宝通汽车服务有限责任公司', '广汇汽车服务有限责任公司东台分公司', '广汇汽车服务有限责任公司天津武清分公司', '上海宝信实嘉汽车销售有限公司天津武清分公司')
GROUP BY  substr(finance_time,1,7),sale_type
) as A
LEFT JOIN
...
SUM(case when install_price is NULL then 0 else install_price end) ) as income,
sum(cost) AS costx 
 from dj_2024_settle_details_record where settle_time >= '2024-01-01' and device_type in (6,12501) and settle_merchant = '广汇汽车'
GROUP BY settle_merchant,DATE_FORMAT(settle_time,'%Y-%m')
)
as B ON (B.cus_name = A.cus_name AND a.monthx = B.monthx) HAVING abs(gapcost) >1.0;
gap <>  0
...
hw_price as '硬件价格', install_price as '安装价格', active_time as '激活时间',cost AS '单位成本' FROM dj_2024_settle_details where device_type =8 
AND settle_time >= '2024-01-01';
...
settle_time as '结算时间', pk_price as '套餐价格', 
hw_price as '硬件价格', install_price as '安装价格', active_time as '激活时间',cost AS '单位成本'  FROM dj_2024_settle_details_record
where settle_time >= '2024-01-01';
...
0-hw_price as '硬件价格', 0-install_price as '安装价格', active_time as '激活时间',cost AS '单位成本'  FROM dj_2024_settle_details_return
WHERE settle_time >= '2024-01-01';
...
'GPS' as '设备类型',
settle_time as '结算时间', pk_price as '套餐价格', 
hw_price as '硬件价格', install_price as '安装价格', active_time as '激活时间',cost AS '单位成本',pk_price+hw_price+install_price AS '单价合计',1 AS '硬件数量','追踪器明细' AS '来源'
FROM dj_2024_settle_details where device_type =8 AND settle_time >= '2024-01-01'
UNION 
SELECT channel as '渠道', settle_merchant as '结算客户',sn as '设备号',
case when device_type = 6 then '记录仪' when device_type = 12501 then '车充' end as '设备类型',
settle_time as '结算时间', pk_price as '套餐价格', 
hw_price as '硬件价格', install_price as '安装价格', active_time as '激活时间',cost AS '单位成本',pk_price+hw_price+install_price AS '单价合计',1 AS '硬件数量','记录仪明细' AS '来源'  
FROM dj_2024_settle_details_record where settle_time >= '2024-01-01'
UNION 
SELECT channel as '渠道', settle_merchant as '结算客户',sn as '设备号', 
case when device_type = 8 then 'GPS' when device_type = 6 then '记录仪' when device_type = 12501 then '车充' end as '设备类型',
settle_time as '结算时间', 0-pk_price as '套餐价格', 
0-hw_price as '硬件价格', 0-install_price as '安装价格', active_time as '激活时间',cost AS '单位成本',(0-pk_price)+(0-hw_price)+(0-install_price) AS '单价合计',-1 AS '硬件数量','退货明细' AS '来源'   
FROM dj_2024_settle_details_return WHERE settle_time >= '2024-01-01';
