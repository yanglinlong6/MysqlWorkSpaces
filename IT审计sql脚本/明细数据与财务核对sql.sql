...
select A.cus_name,
       ...
       A.total                  '财务数量',
       B.total                  '商务数量',
       (A.total - B.total)   as gap,
       A.income              as '财务收入',
       b.income              as '商务收入',
       (A.income - b.income) as gapincome
FROM (
         SELECT TRIM(cus_name)                                           as cus_name,
         ...
         LEFT JOIN
     (
         SELECT settle_merchant                                                      as cus_name,
...
SELECT *
FROM dj_2022_settlment_order_finance_06plus
where finance_time >= '2022-07-01'

    ('东台宝通汽车服务有限责任公司', '广汇汽车服务有限责任公司东台分公司')


# 追踪器的
SELECT channel         as '渠道',
       settle_merchant as '结算客户',
       sn              as '设备号',
       settle_time     as '结算时间',
       pk_price        as '套餐价格',
       hw_price        as '硬件价格',
       install_price   as '安装价格',
       active_time     as '激活时间'
...
SELECT channel                                                                                                     as '渠道',
       ...
       sn                                                                                                          as '设备号',
       case
           when device_type = 8 then '记录仪'
           when device_type = 6 then '记录仪'
           when device_type = 12501
               then '车充' end                                                                                       as '设备类型',
       settle_time                                                                                                 as '结算时间',
       0 - pk_price                                                                                                as '套餐价格',
       0 - hw_price                                                                                                as '硬件价格',
       0 - install_price                                                                                           as '安装价格',
       active_time                                                                                                 as '激活时间'
FROM dj_2022_settle_details_return;
...
FROM dj_2021_settle_details
where settle_time >= '2022-03-01'
GROUP BY device_type;



SELECT is_virtual, COUNT(1)
from dj_2022_settle_details_record
where DATE_FORMAT(settle_time, '%Y-%m') = '2022-12'
GROUP BY is_virtual
    '''
...
SELECT settle_merchant, DATE_FORMAT(settle_time, '%Y-%m'), COUNT(1)
FROM dj_2021_settle_details
where device_type = 8
...
SELECT *
from dj_2022_settlment_order_finance_0304
where cus_name = '四川汇鹏兴成汽车服务有限公司'
  and device_type in ('记录仪', '车充')