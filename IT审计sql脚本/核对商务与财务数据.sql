...
FROM (
         SELECT TRIM(cus_name)                                           as cus_name,
                ...
                SUM(cost)
         FROM dj_2022_settlment_order_finance
         ...
         LEFT JOIN
     (
         SELECT cus_name, monthx, SUM(tmp.totalx) as total, sum(tmp.incomex) as income
         from (
                  SELECT TRIM(cus_name)                                                 as cus_name,
                         substr(finance_time, 1, 7)                                     as monthx,
                         sale_type,
...
FROM (
         SELECT TRIM(cus_name)                                           as cus_name,
                DATE_FORMAT(finance_time, '%Y-%m')                       as monthx,
                sale_type,
                SUM(case when product_type = '硬件' then total else 0 end) as total,
                SUM(income)                                              as income,
                SUM(cost)
     ...
     (
         ...
         from (
                  SELECT TRIM(cus_name)                                                                          as cus_name,
                         ...
                         sale_type,
                         (case when product_type = '硬件' then total else 0 end)                                   as totalx,
                         (total * (IFNULL(hw_unit_price, 0) + IFNULL(pk_unit_price, 0) +
                                   IFNULL(install_unit_price, 0)))                                               as incomex
