...
SUM(case when product_type='硬件' then total else 0 end) as total,SUM(income) as income,SUM(cost) FROM dj_2022_settlment_order_finance
where  device_type in ('记录仪','车充') and product_type in ('硬件','服务','安装') and business_type ='智慧门店SAAS服务'
...
where  device_type in ('记录仪','车充') and product_type in ('硬件') and business_type ='智慧门店SAAS服务'
) tmp
...
SELECT TRIM(cus_name) as cus_name, DATE_FORMAT(finance_time, '%Y-%m') as monthx, sale_type,
(case when product_type='硬件' then total else 0 end) as totalx,
(total*(IFNULL(hw_unit_price,0) + IFNULL(pk_unit_price,0)+IFNULL(install_unit_price,0) )) as incomex
